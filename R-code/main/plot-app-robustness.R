...
files_mse <-
    c(
        list.files(root_folder,
            pattern = "mses\\.csv", recursive = TRUE,
            full.names = TRUE
        ),
        list.files(root_folder2,
            pattern = "mses\\.csv", recursive = TRUE,
            full.names = TRUE
        )
    )
...
lab_vec <- methods_map$label
names(lab_vec) <- methods_map$code
col_vec <- methods_map$color
names(col_vec) <- methods_map$code
shape_vec <- methods_map$shape
names(shape_vec) <- methods_map$code
linetype_vec <- methods_map$linetype
names(linetype_vec) <- methods_map$code
...
mses1 <- mses %>%
    ...
    mutate(
        max_mse = if_else(model %in% c("Causal", "IMP"), mse, max_mse),
...
gg <- ggplot(
    ...
    aes(
        x = int_par, y = mse, color = model_, fill = model_,
    ...
    labs(
        x = "Perturbation Strength", y = "MSE", color = "Methods",
        fill = "Methods", shape = "Methods", linetype = "Methods"
    ) +
    theme(legend.position = "none") +
    xlab("") +
    ylab("") +
    coord_cartesian(ylim = c(0.3, 4.75))
gg

save_myplot(
    plt = gg, plt_nm = "../results/figures/app-robustness-n-3000.pdf",
...
gg <- ggplot(
    mses_BCF,
    aes(
        x = int_par, y = mse, color = model, fill = model,
