...
def main():
    ...
    for i in range(cfg.n_sims):
        # %%
        print(f"Processing simulation {i+1}/{cfg.n_sims}")

        all_data = []  # List to collect all dataframes

        for int_strength in cfg.interv_strength:
            ...
            for algo_name, algo in cfg.algorithms[:]:
                ...
                # Create column names
                column_names = [f"X{i+1}" for i in range(d)]

                # Convert to pandas DataFrame
                df_X = pd.DataFrame(X_combined, columns=column_names)

                combined_df = pd.concat(
                    [
                        pd.DataFrame(
                            {
                                "set": ["train"] * len(X_train)
                                + ["test"] * len(X_test),
                                "algorithm": [algo_name] * data_length,
                                "interv_strength": [int_strength] * data_length,
